...
class Weglot {
	...
	public function init_function() { 
		...
		if(!$areRulesOK || (get_option("frew")=="on")) { 
			$this->updateRewriteRule();
		}
	...
	public function plugin_settings() {
		...
		register_setting( 'my-plugin-settings-group', 'frew' );
	}
	...
	public function isEligibleURL($url) {
		...
		$regex = get_option("exclude_url");  

		if($regex!="" && preg_match('/'.str_replace('/', '\/',$regex).'/',$url)==1)
			return false;
		else
			return true;
	}
	...
	public function returnWidgetCode($forceNoMenu = false) { 
		...
		$aside1 = ($is_menu && !$is_dropdown) ? '':'<aside id="weglot_switcher" wg-notranslate class="'.$moreclass.'country-selector closed" onclick="openClose(this);" >';
