...
class Options_Machine {
    ...
    public static function optionsframework_machine($options) {
        ...
        foreach ($options as $value) {
            ...
            /* condition start */
            if (count($smof_data) > 1 || count($data) > 1) {
                ...
                switch ($value['type']) {
                    ...
                    case 'images':
                        ...
                        foreach ($value['options'] as $key => $option) {
                            ...
                            $output .= '<img data-rel="of-radio-img-' . $value['id'] . $i . '" src="' . esc_url($option) . '" alt="" class="of-radio-img-img ' . $selected . '" />';
                            $output .= '</span>';
                        }
                    ...
                    case 'import_button':
                        ...
                        foreach ($import_types['post-type'] as $data_key => $data_value) {
                            $output .= '<div class="iw-data-option"><input class="post-type" name="post-type-' . $data_key . '" type="checkbox" checked="checked" value="1" /> ' . $data_value . '</div>';
                        }
                        foreach ($import_types['data-type'] as $data_key => $data_value) {
                            $output .= '<div class="iw-data-option"><input class="data-type" name="data-type-' . $data_key . '" type="checkbox" checked="checked" value="1" /> ' . $data_value . '</div>';
                        }
                        $output .= '</div>';
                        $output .= '<div class="system-required">';
                        $output .= '<h3>' . __('System Requirements', 'inwavethemes') . '</h3>';
                        $output .= self::getSystemRequireStatus();
                        $output .= '</div>';
                        $output .= '<div style="clear:both;"></div>';
                        $output .= '</div>';
                        $output .= '<div class="iw-import-button"><a href="' . esc_url($src) . '" class="button-primary">' . $value['btntext'] . '</a></div>';
                    ...
                    case 'backup':
                        ...
                        $output .= '<p><strong>' . __('Last Backup : ', 'inwavethemes') . '<span class="backup-log">' . $log . '</span></strong></p></div>' . "\n";
    ...
    public static function optionsframework_media_uploader_function($id, $std, $mod) {
        ...
        $uploader = '';
	    $upload = isset($smof_data[$id]) ? $smof_data[$id] : '';
        $hide = '';
        ...
        if (!empty($upload)) {
            ...
            $uploader .= '<img style="background:grey;" class="of-option-image" id="image_' . $id . '" src="' . esc_url($imageurl) . '" alt="" />';
            $uploader .= '</a>';
        }
        $uploader .= '</div>';
    ...
    public static function optionsframework_slider_function($id, $std, $oldorder, $order) {
        ...
        $slider = '';
        $slide = array();
	    $slide = isset($smof_data[$id]) ? $smof_data[$id] : '';
    ...
    static function getSystemRequireStatus() {
        ...
        $file_get_content = @file_get_contents('http://inwavethemes.com/wordpress/index.htm');
        ?>
        <ul class="check-system-status">
            <li>
                <?php
                echo __('<span class="label1">PHP Version: </span>', 'inwavethemes');
                if ($php_version < '5.3.0') {
                    echo '<span class="invalid"><i class="fa fa-times"></i></span>' . sprintf(__('<span class="label2">Currently: %s</span>', 'inwavethemes'), $php_version) . '(' . __('Min: 5.3', 'inwavethemes') . ')';
                } else {
                    echo '<span class="valid"><i class="fa fa-check"></i></span>' . sprintf(__('<span class="label2">Currently: %s</span>', 'inwavethemes'), $php_version);
                }
                ?>
            </li>
            <li>
                <?php
                echo __('<span class="label1">Maximum execution time: </span>', 'inwavethemes');
                if ($max_execution_time < '90') {
                    echo '<span class="invalid"><i class="fa fa-times"></i></span>' . sprintf(__('<span class="label2">Currently: %s</span>', 'inwavethemes'), $max_execution_time) . '(' . __('Min: 90', 'inwavethemes') . ')';
                } else {
                    echo '<span class="valid"><i class="fa fa-check"></i></span>' . sprintf(__('<span class="label2">Currently: %s</span>', 'inwavethemes'), $max_execution_time);
                }
                ?>
            </li>
            <li>
                <?php
                echo __('<span class="label1">Memory Limit: </span>', 'inwavethemes');
                if ($memory_limit < '128M') {
                    echo '<span class="invalid"><i class="fa fa-times"></i></span>' . sprintf(__('<span class="label2">Currently: %s</span>', 'inwavethemes'), $memory_limit) . '(' . __('Min: 128M', 'inwavethemes') . ')';
                } else {
                    echo '<span class="valid"><i class="fa fa-check"></i></span>' . sprintf(__('<span class="label2">Currently: %s</span>', 'inwavethemes'), $memory_limit);
                }
                ?>
            </li>
            <li>
                <?php
                echo __('<span class="label1">Uploads folder writable: </span>', 'inwavethemes');
            ...
            <li>
                <?php
                echo __('<span class="label1">Connect InwaveThemes Server: </span>', 'inwavethemes');
                if (!$file_get_content) {
                    echo '<span class="invalid"><i class="fa fa-times"></i></span>';
                } else {
                    echo '<span class="valid"><i class="fa fa-check"></i></span>';
            ...
            <li>
                <?php
                echo __('<span class="label1">Start Session: </span>', 'inwavethemes');

                if (!session_id()) {
                    session_start();
                }
                if(!session_id()){
                    echo '<span class="invalid"><i class="fa fa-times"></i></span>' . __('<span class="label2">Failed to start session. Please check session save_path in your php setting</span>', 'inwavethemes');
                }else {
                    echo '<span class="valid"><i class="fa fa-check"></i></span>';
                }
                ?>
