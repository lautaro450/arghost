...
function inwave_scripts() {
    ...
    /* load whmcs style */
	$whmp_page_id = inwave_get_whmp_page_id();
    if (is_page() && (get_option('cc_whmcs_bridge_pages') == get_the_ID() || ($whmp_page_id == get_the_ID()) )) {
...
/** get WHMPress Client Area Page ID */
function inwave_get_whmp_page_id() {
	if(class_exists('WHMPress_Client_Area')){
		$whmp_ca = new WHMPress_Client_Area;
	}
	if(class_exists('WHMCS_Client_Area')){
		$whmp_ca = new WHMCS_Client_Area;
	}
	if(!empty($whmp_ca)){
		$whmp_page_id = $whmp_ca->get_client_area_page_id();
	}else{
		// fixed id
		$whmp_page_id = 28;
	}
